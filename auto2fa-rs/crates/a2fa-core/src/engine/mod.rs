...
//! # Subscriber model
//!
//! Change events are pushed to subscribers via `std::sync::mpsc::Sender<String>`
//! channels.  Each subscriber holds the `Sender` end; the engine holds a `Vec`
//! of clones.  Dead (disconnected) senders are pruned lazily inside `emit`.
//! The `String` payload is a JSON-encoded IPC event (same wire format as
//! `daemon.py`'s `_emit`).
...
pub struct State {
    ...
    /// Active event subscribers. Each entry is an `mpsc::Sender` whose
    /// consumer (typically a connected IPC client) receives JSON event strings.
    /// Dead senders are pruned lazily inside [`State::emit`].
...
impl State {
    ...
    /// Push a JSON event string to all live subscribers, pruning dead ones.
    pub fn emit(&mut self, event_json: String) {
        self.subscribers
            .retain(|tx| tx.send(event_json.clone()).is_ok());
    }

    /// Add a subscriber channel.
    pub fn subscribe(&mut self, tx: Sender<String>) {
