    ...
    while attempt < retries:
        try:
            ...
            if index == pexpect.TIMEOUT:
                ...
                if keep_alive:
                    maintain_ssh_session(child, host, password, otp)  # Keep the session alive with reconnect
                return True
...
def maintain_ssh_session(child, host, password, otp):
    ...
    try:
        while True:
            # Send a keep-alive command or just wait for user input
            ...
            if index == pexpect.EOF:
                ...
                ssh_login_with_interactive_prompt(host, password, otp)  # Reconnect on disconnection
                break
        ...
        ssh_login_with_interactive_prompt(host, password, otp)  # Reconnect on error
