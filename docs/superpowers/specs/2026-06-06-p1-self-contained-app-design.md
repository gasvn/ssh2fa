...
external repo + `.venv` (via `~/.ssh2fa/project-dir.txt` / `python-path.txt`)
...
We want a **self-contained `SSH2FA.app`**: the recipient drags it to
`/Applications`, opens it, and it works — no Python, no repo, no manual setup.

## Goal

`SSH2FA.app` ships with its own Python daemon embedded. On first launch the app
...
`xattr -dr com.apple.quarantine SSH2FA.app`).
...
SSH2FA.app/
  Contents/
    MacOS/SSH2FA                                   Swift app (unchanged entry)
    Resources/daemon/ssh2fa-daemon                 PyInstaller --onedir output:
    Resources/daemon/_internal/...                  embedded CPython + deps
    Library/LaunchAgents/com.ssh2fa.daemon.plist   SMAppService agent plist
```

- **Bundled path (end user):** at launch the app detects it is a bundled build
  (the embedded daemon exists at `Contents/Resources/daemon/ssh2fa-daemon`) and
  registers the agent via `SMAppService.agent(plistName: "com.ssh2fa.daemon.plist").register()`.
...
- **Source path (developer):** when there is no embedded daemon (running the
  ...
  `~/.ssh2fa/project-dir.txt` and spawns `python -m auto2fa.daemon` using the
...
`Contents/Library/LaunchAgents/com.ssh2fa.daemon.plist`, using `BundleProgram`
(a bundle-relative path), NOT an absolute `ProgramArguments` path:

```xml
<key>Label</key>            <string>com.ssh2fa.daemon</string>
<key>BundleProgram</key>    <string>Contents/Resources/daemon/ssh2fa-daemon</string>
<key>RunAtLoad</key>        <true/>
<key>KeepAlive</key>        <dict><key>SuccessfulExit</key><false/></dict>
```

(No `SSH_CONFIG_PATH` needed — `credentials.config_dir()` falls back to `~/.ssh`.
`StandardOut/ErrorPath` may point at `/tmp/ssh2fa_daemon.log` as today.)

## Bundling approach

**PyInstaller `--onedir`** builds a standalone `ssh2fa-daemon` (embedded CPython
+ deps) from the `ssh2fa-daemon` console entry point. Output `dist/ssh2fa-daemon/`
...
1. **PyInstaller build** — a spec or a `build.sh` step that produces
   `dist/ssh2fa-daemon/` with the hidden imports above, excluding textual.
...
1. **Bundled daemon runs in a clean env:** after PyInstaller build,
   `env -i HOME=$HOME PATH=/usr/bin:/bin Contents/Resources/daemon/ssh2fa-daemon`
   starts, binds the socket, and `list_hosts` returns data. This proves the
   embed has no missing imports (esp. keyring).
2. **Bundled app e2e:** build `SSH2FA.app`; from a clean shell confirm the app
   registers the agent (`launchctl print gui/$UID/com.ssh2fa.daemon` or
...
- **P1a — bundle the daemon:** PyInstaller build + embed in `.app` + build
  pipeline. Done when the embedded `ssh2fa-daemon` runs in a clean env from
...
- **P1b — Swift integration + polish:** SMAppService agent registration +
  ...
  docs. Done when a freshly built `SSH2FA.app`, launched on a clean account,
